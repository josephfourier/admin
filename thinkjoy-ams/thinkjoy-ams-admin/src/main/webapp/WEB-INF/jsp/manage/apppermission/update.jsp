	...
	<form id="updateForm" method="post">
		<div class="radio">
			<div class="radio radio-inline radio-success">
				<input id="type_0" type="radio" name="type" value="0" <c:if test="${permission.type==0}">checked</c:if>>
			...
			<span class="type0 type1 type2 type3">
				<select id="systemId" name="systemId">
					...
					<c:forEach var="amsApp" items="${amsApps}">
						<option value="${amsApp.appId}" <c:if test="${permission.systemId==amsApp.appId}">selected="selected"</c:if>>${amsApp.appName}</option>
					</c:forEach>
				</select>
			</span>
		...
		<div class="form-group sl type2 type3" hidden>
			...
			<select class="form-control" id="isApproval" name="isApproval" >
				...
				<c:forEach items="${dict:getValueByCode(isApproval)}" var="at">
					<option <c:if test="${at.valueKey eq permission.isApproval}">selected</c:if> value="${at.valueKey}">${at.valueName}</option>
				</c:forEach>
			</select>
		</div>
		<div class="form-group type2 type3" hidden>
			<label for="approvalUri">审批页面地址</label>
			<input id="approvalUri" type="text" class="form-control" name="approvalUri"  value="${permission.approvalUri}">
		</div>
		<div class="form-group sl type2 type3" hidden >
			...
			<select id="applicableIdentity" name="applicableIdentity" multiple="multiple" style="width:200px;">
				...
				<c:forEach items="${dict:getValueByCode(applicableIdentity)}" var="at">
					<option value="${at.valueKey}"
							<c:forEach var="applicableIdentityList" items="${applicableIdentityList}"><c:if test="${at.valueKey eq applicableIdentityList}">selected="selected"</c:if></c:forEach>>${at.valueName}</option>
				</c:forEach>
			</select>
		</div>
...
<script>
	...
	function updateSubmit() {
		//console.log("是否有审批流程："+$('#isApproval').val());
		...
		$.ajax({
			type: 'post',
			url: '${basePath}/manage/apppermission/update/${permission.permissionId}',
