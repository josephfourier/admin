...
　　`thinkjoy`项目创建于2017年7月12日，正在慢慢成长中，目的不仅仅是一个开发架构，而是努力打造一套从 **前端模板** - **基础框架** - **分布式架构** - **开源项目** - **持续集成** - **自动化部署** - **系统监测** - **无缝升级** 的全方位J2EE企业级开发解决方案。
...
``` lua
thinkjoy
├── thinkjoy-common -- SSM框架公共模块
├── thinkjoy-upms -- 用户权限管理系统
|    ├── thinkjoy-upms-common -- upms系统公共模块
|    ├── thinkjoy-upms-dao -- 代码生成模块，无需开发
|    ├── thinkjoy-upms-client -- 集成upms依赖包，提供单点认证、授权、统一会话管理
|    ├── thinkjoy-upms-rpc-api -- rpc接口包
|    ├── thinkjoy-upms-rpc-service -- rpc服务提供者
|    └── thinkjoy-upms-server -- 用户权限系统及SSO服务端[端口:1111]
├── thinkjoy-ucenter -- 用户中心系统
├── thinkjoy-ams -- 应用管理系统
...
thinkjoy.jprogress.js | 加载进度条插件  | [https://github.com/shuzheng/thinkjoy.jprogress.js](https://github.com/shuzheng/thinkjoy.jprogress.js)
thinkjoy.jtotop.js | 返回顶部插件  | [https://github.com/shuzheng/thinkjoy.jtotop.js](https://github.com/shuzheng/thinkjoy.jtotop.js)
...
基于bootstrap实现的响应式Material Design风格的通用后台管理系统，`thinkjoy`项目所有后台系统都是使用该模块界面作为前端展示。
...
![API网关](project-bootstrap/thinkjoy-api.png)
...
![统一扫码支付](project-bootstrap/thinkjoy-pay.png)
...
[环境搭建和系统部署文档(作者：小兵)](http://git.oschina.net/shuzheng/thinkjoy/attach_files "环境搭建和系统部署文档(作者：小兵)")
...
- 模块命名为`项目`-`子项目`-`业务`，如`thinkjoy-cms-admin`

- 数据表命名为：`子系统`_`表`，如`cms_article`

- 更多规范，参考[[阿里巴巴Java开发手册] http://git.oschina.net/shuzheng/thinkjoy/attach_files

```

### 数据模型
![数据库模型](project-datamodel/thinkjoy.png)
...
- thinkjoy-cms-admin启动卡住：因为没有启动activemq

- thinkjoy-upms-server访问报session不存在：因为没有启动redis服务
