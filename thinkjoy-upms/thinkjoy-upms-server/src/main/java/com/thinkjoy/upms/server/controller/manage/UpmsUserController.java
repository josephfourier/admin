...
import com.thinkjoy.upms.rpc.api.*;
...
public class UpmsUserController extends BaseController {
    ...
    @RequestMapping(value = "/agencytree/{uid}", method = RequestMethod.POST)
    @RequiresPermissions("upms:user:read")
    @ResponseBody
    public Object agency(@PathVariable("uid") String id){
        UpmsUser upmsUser = upmsUserService.selectByPrimaryKey(Integer.parseInt(id));
        List<String> codes = new ArrayList<>();
        if (upmsUser != null){
            codes.add(upmsUser.getRelationCode());
        }
	...
	@ApiOperation(value = "打开用户学校页面")
	@RequiresPermissions("upms:user:school")
	@RequestMapping(value = "/school/{id}", method = RequestMethod.GET)
	public String school(@PathVariable("id") int id, ModelMap modelMap) {
		// 所有学校
		List<UcenterSchool> ucenterSchools = ucenterSchoolService.selectByExample(new UcenterSchoolExample());
		// 用户拥有学校
		UpmsUserSchoolExample upmsUserSchoolExample = new UpmsUserSchoolExample();
		upmsUserSchoolExample.createCriteria()
				.andUserIdEqualTo(id);
		List<UpmsUserSchool> upmsUserSchools = upmsUserSchoolService.selectByExample(upmsUserSchoolExample);
		modelMap.put("ucenterSchools",ucenterSchools);
		modelMap.put("upmsUserSchools",upmsUserSchools);
		modelMap.put("userId",id);
		return "/manage/user/school.jsp";
	}

	@ApiOperation(value = "用户学校")
	@RequiresPermissions("upms:user:school")
	@RequestMapping(value = "/school", method = RequestMethod.POST)
	@ResponseBody
	public Object school( @RequestParam(required = false, defaultValue = "10", value = "id") String  id,
						  HttpServletRequest request) {
		Integer userId=Integer.parseInt(id);
		String[] schoolCodes = request.getParameterValues("schoolCode");
		upmsUserSchoolService.school(schoolCodes, userId);
		return new UpmsResult(UpmsResultConstant.SUCCESS, "");
	}
