...
import com.thinkjoy.upms.client.shiro.session.UpmsSession;
...
public class SSOController extends BaseController {
    ...
    public String login(HttpServletRequest request) {
        ...
        // 判断是否已登录，如果已登录，则回跳
        String code = RedisUtil.get(THINKJOY_UPMS_SERVER_SESSION_ID + "_" + serverSessionId);
        String codeValue = RedisUtil.get(THINKJOY_UPMS_SERVER_CODE + "_" + code);

        //防止sessionid存在,而code不存在的状况,这里重新设置一下
        if (StringUtils.isNotBlank(code) && StringUtils.isBlank(codeValue)){
            // code校验值
            RedisUtil.set(THINKJOY_UPMS_SERVER_CODE + "_" + code, code, (int) subject.getSession().getTimeout() / 1000);
        }
        // code校验值
        if (StringUtils.isNotBlank(code) && StringUtils.isNotBlank(codeValue)) {
            // 回跳
            String backurl = request.getParameter("backurl");
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        String hasCode = RedisUtil.get(THINKJOY_UPMS_SERVER_SESSION_ID + "_" + sessionId);
        // code校验值
        if (StringUtils.isBlank(hasCode)) {
            ...
            RedisUtil.lpush(THINKJOY_UPMS_SERVER_SESSION_IDS, sessionId.toString());
            // 默认验证帐号密码正确，创建code
            String code = UUID.randomUUID().toString();
            // 全局会话的code
            RedisUtil.set(THINKJOY_UPMS_SERVER_SESSION_ID + "_" + sessionId, code, (int) subject.getSession().getTimeout() / 1000);
            // code校验值
            RedisUtil.set(THINKJOY_UPMS_SERVER_CODE + "_" + code, code, (int) subject.getSession().getTimeout() / 1000);
        }
        // 回跳登录前地址
    ...
    public Object code(HttpServletRequest request) {
        ...
        String code = RedisUtil.get(THINKJOY_UPMS_SERVER_CODE + "_" + codeParam);
