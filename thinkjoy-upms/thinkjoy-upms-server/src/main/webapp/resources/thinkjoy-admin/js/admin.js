...
$(function() {
	...
	// 显示cookie菜单
	if(MANAGER_TYPE=="3"){
		if($.cookie('thinkjoy-upms-systemid')==1){
			$.cookie('thinkjoy-upms-systemid',2);
			$.cookie('thinkjoy-upms-systemname','thinkjoy-ucenter-admin');
			$.cookie('thinkjoy-upms-systemtitle','用户中心管理系统');
		}
		var systemid = $.cookie('thinkjoy-upms-systemid') || 2;
		var systemname = $.cookie('thinkjoy-upms-systemname') || 'thinkjoy-ucenter-admin';
		var systemtitle = $.cookie('thinkjoy-upms-systemtitle') || '用户中心管理系统';
	}else{
	...
	if(BACK_URL!=""){
		var index = BACK_URL.replace(/\./g, '_').replace(/\//g, '_').replace(/:/g, '_').replace(/\?/g, '_').replace(/,/g, '_').replace(/=/g, '_').replace(/&/g, '_');
		Tab.addTab(BACK_URL_TITLE,BACK_URL);
