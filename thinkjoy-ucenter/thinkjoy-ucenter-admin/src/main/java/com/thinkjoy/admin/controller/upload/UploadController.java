...
import com.thinkjoy.admin.controller.upload.service.UploadService;
...
public class UploadController extends BaseController{
    ...
    public Object uploadDatas(HttpServletRequest request,HttpServletResponse response,
        ...
        if(schoolCode==null){
            request.getSession().setAttribute("isFinished", true);//是否导入完成：否
        ...
        if(excelHead==null||(excelHead.size()>0&&tableValid.getColNum()!=excelHead.get(0).size())){
            request.getSession().setAttribute("isFinished", true);//是否导入完成：否
        ...
        try {
            uploadDatas = getData(request, tableValid.getColNum());
            ...
            if(uploadDatas!=null&&uploadDatas.size()>0){
                //调用对应的方法
                if(BaseConstants.ImportModel.BASEMODEL_STUDENT.equals(baseModel)){
                    errorMap = uploadService.importUcenterStudents(uploadDatas,ucenterSchool,request,tableValid);
                }else if(BaseConstants.ImportModel.BASEMODEL_TEACHER.equals(baseModel)){
                    errorMap = uploadService.importUcenterTeacher(uploadDatas,ucenterSchool,request,tableValid);
                }else{
                    request.getSession().setAttribute("isFinished", true);//是否导入完成：否
