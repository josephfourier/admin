    ...
    <form id="importFile" name="importFile" class="form-horizontal" method="post" enctype="multipart/form-data">
      <div class="box-body">
        <div>
          ...
          <input id="excelFile" name="excelFile" class="file-loading" type="file" multiple accept=".xls,.xlsx" onclick="resetMessage()"> <br>
        </div>
      </div>
    </form>
  </div>
...
//  var my_interval;
//  //上传按钮
//  $(document).off('click').on('click', '.fileinput-upload-button', function(){
//    //轮询  查导入结果信息
//    my_interval = setInterval('refreshImportDatas()',1000);//轮询执行，1000ms一次
//    console.log("轮询id："+my_interval);
//  });
  ...
  function refreshImportDatas(){
    $.ajax({
      ...
      success: function (result) {
        ...
        if(result.isFinished==true){//完成时结束轮询
          clearInterval(my_interval);
          resetImportDatas();//重置导入进度信息
          console.log("完成时结束轮询："+my_interval);
          $('.jconfirm-closeIcon').show();
        }
//        if(result.isFinished=="-1"){//为空结束轮询
//          clearInterval(my_interval);
//          console.log("为空结束轮询："+my_interval);
//        }
