...
JAVA_OPTS="-Dfile.encoding=UTF-8 -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -Ddubbo.registry.file=$LOCK_ZOOKEEPER_FILE"
JAVA_DEBUG_OPTS=""
if [ "$1" = "debug" ]; then
...
#    COUNT=`ps -f | grep java | grep "$DEPLOY_DIR" | awk '{print $2}' | wc -l`
#    fi
    COUNT=`netstat -tnlp|grep 2088|wc -l`
    if [ $COUNT -gt 0 ]; then
        break
    fi
done
