(function (factory) {
    ...
    var CityPicker = function (el, opts, fun) {
        var options = {};
        ...
        if (options.remote.pro) {
            var that = this;
            $.getJSON(options.remote.pro, function (result) {
                that.provinces = result;
                that.init();
            });
        } else {
            this.provinces = options.provinces || provinces;
            this.init();
        }
    };
    ...
    $.extend(CityPicker.prototype, {
        ...
        createProvincesList: function() {
            var provinces = this.provinces;
            var dl = '';

            for (var key in provinces) {
                var areaCode = key.split('_')[0];
                var areaType = key.split('_')[1];
                var dt = '<dt id="' + areaCode + '">' + areaCode + '</dt>';

                var dd = '';
                    dd += '<dd data-areacode="' + areaCode + '" data-areatype="' + areaType +'">' + provinces[key] + '</dd>'
                dl += '<dl>' + dd + '</dl>';
            }
            $('.picker-box').append('<section class="pro-picker">' + dl + '</section>');
        },

        createCityList: function(url, areaCode, areaType) {
            var that = this;
            $.getJSON(url, {areaCode: areaCode, areaType: 2, limit: 10000}, function(result) {

                var cityList = result.rows;
                var ul, li = '';

                $.each(cityList, function() {
                    li += '<li data-citycode="' + this.areaCode + '">' + this.areaName + '</li>'
                });
                ul = '<ul class="city-picker">' + li + "</ul>";

                $('.pro-picker').hide();

                $('.picker-box').find('.city-picker')
                    .remove().end()
                    .append(ul);
                that.cityClick();
            });
        ...
        provinceClick: function() {
            ...
            $('.pro-picker').on('click', function (e) {
                ...
                if ($target.is('dd')) {
                    ...
                    that.proCode = $target.data('areacode');
                    var areaType = $target.data('areatype');
                    that.createCityList(that.options.remote.city, that.proCode, areaType);

                    $.extend(!0, that.hiddenList, {
                        province    :   that.pro,
                        provinceCode:   that.proCode
                    });
                    //$(this).hide();
                }
            })
        ...
        cityClick: function() {
            ...
            $('.city-picker').on('click', function(e) {
                ...
                if ($target.is('li')) {
                    ...
                    that.cityCode = $target.data('citycode');

                    if (that.elType) {
                        that.el.val(that.pro + "-" + that.city);
                    } else {
                        that.el.html('<span style="color: #555555">' + that.pro + "-" + that.city + "</span>");
                    }
        ...
        cleanup: function() { this.hiddenList = {}; },

        createCityPickerBox: function() {
          $('body').append('<div class="picker-box"></div>');
        },
    ...
    $.fn.cityPicker = function(options, fun) {
        return new CityPicker(this, options, fun);
    }
});