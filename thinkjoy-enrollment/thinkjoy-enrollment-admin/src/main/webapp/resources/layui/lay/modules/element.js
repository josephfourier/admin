layui.define('jquery', function(exports){
  ...
  ,NAV_ANIM = 'layui-anim layui-anim-upbit', NAV_CURRENT = 'lap-nav-current'
  ...
  ,call = {
    ...
    ,showChild: function(){
      ...
      if(parents.hasClass(NAV_TREE)){
        ...
        if(othis.parent().is('dl')) {
          parents.find('.'+THIS).removeClass(THIS);
        }
        if (!parent.hasClass(NAV_ITEM + 'ed')) {
          parents.find('.' + NAV_ITEM + 'ed').removeClass(NAV_ITEM+'ed');
        }
      ...
      ,nav: function(){
        ...
        $(NAV_ELEM + elemFilter).each(function(index){
          ...
          itemElem.each(function(){
            ...
            //二级菜单
            if(child[0] && (showMore = child.find('dd').length > 1) && !oitem.find('.'+NAV_MORE)[0]){
              var one = oitem.children('a');
              one.append('<span class="'+ NAV_MORE +'"></span>');
            }

            oitem.off('click', call.clickThis).on('click', call.clickThis); //点击选中
            if (showMore) oitem.children('a').off('click', call.showChild).on('click', call.showChild); //展开二级菜单
            else {
              oitem.off('click').on('click', function() {
