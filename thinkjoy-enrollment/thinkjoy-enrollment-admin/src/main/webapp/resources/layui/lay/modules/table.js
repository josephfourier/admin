layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  , ELEM_SELECTD = 'lap-table-selected' // 单击行选中
  //thead区域模板
  ...
  Class.prototype.setCheckData = function(index, checked){
    ...
    //添加选中当前行
    if (thisData[index][config.checkName]) {
    	that.layBody.find('tr:eq(' + index + ')').addClass(ELEM_SELECTD);
    } else {
    	that.layBody.find('tr:eq(' + index + ')').removeClass(ELEM_SELECTD);
    }
  };
  ...
  Class.prototype.events = function(){
   ...
   /* that.layBody.on('click', 'tr', function() {
      var self = $(this)
          , index = self.index();
      if (self.hasClass(ELEM_SELECTD)) {
        self.removeClass(ELEM_SELECTD);
        return ;
      }
      that.layBody.find('tr').removeClass(ELEM_SELECTD).end().find('tr:eq(' + index + ')').addClass(ELEM_SELECTD);
    });*/
